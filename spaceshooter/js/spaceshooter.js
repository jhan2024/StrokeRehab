...
// Set up input listeners (Force input is handled implicitly via window.latestNormalizedForce)
function setupInputListeners() {
    // Remove the previous global listener code entirely
    // We might add listeners specific to the canvas later if needed, but not global window listeners.
    console.log("Space Shooter setupInputListeners called (no global listeners added).");
}
...
function gameLoop(timestamp) {
    ...
    // Continue the game loop and store the ID
...
function stopGame() {
    ...
    // We need to store the request ID to cancel it
    ...
    // Optional: Clear canvas or show a message if needed when explicitly stopped
    // if (ctx) {
    //     ctx.clearRect(0, 0, canvas.width, canvas.height);
...
function updateShip(deltaTime) {
    ...
    // Calculate target Y position based on force (0 force = bottom, 1 force = top)
    // Subtracting from canvas.height because Y=0 is the top
    let targetY = canvas.height - (ship.height / 2) - (forceInput * availableHeight);

    // Invert control if configured (0 force = top, 1 force = bottom)
    if (gameConfig.invertControl) {
        targetY = (ship.height / 2) + (forceInput * availableHeight);
    }

    // Keep target within bounds
    targetY = Math.max(ship.height / 2, Math.min(canvas.height - ship.height / 2, targetY));

    // Smoothly move towards the target Y position (Lerp)
    const lerpFactor = 0.1; // Adjust for desired responsiveness
    ship.y += (targetY - ship.y) * lerpFactor;
    
    // Direct mapping alternative (less smooth):
    // ship.y = targetY; 
}
...
function gameOver() {
    ...
    // Show controls help (optional, might be redundant)
    // document.getElementById('controls-help').style.display = 'block';
    
    // --- Add listener for restart --- 
    const restartHandler = (e) => {
        if (e.code === 'Space') {
            window.removeEventListener('keydown', restartHandler); // Remove this specific listener
            console.log("Restarting Space Shooter...");
            initGame(); // Re-initialize the game state
            startGame(); // Start the loop again - USE RENAMED FUNCTION
        }
    };
    // Use timeout to prevent immediate restart if space was held down
    setTimeout(() => {
        window.addEventListener('keydown', restartHandler, { once: false }); // Add listener after a short delay
    }, 100); 
}
...
// window.isSpaceShooterRunning = function() {
